...
class ApiController extends Controller {
    ...
    {
        // Find out which method called the getRequest method,
        // Whether it was the store or update method.
        $methodName = debug_backtrace()[1]['function'];

        // Check the custom form request classes set in the child controller classes
    ...
    public function store()
    {
        ...
        if ($response instanceof PKResponseResourceCreateError) {
            ...
            $processedItem = $this->processItem($response->resource) ?? $response->resource;
            return $this->apiManager->respond($processedItem->toArray());
        }
        else{
     ...
     * @return array
     */
    public function processItemsWithTransformer($items, $transformer)
    {
        $itemsResource = new Collection($items, $transformer);
        return $this->fractalManager->createData($itemsResource)->toArray();
    }
     ...
     * @return array
     */
    public function processItemWithTransformer($item, $transformer)
    {
        if(isset($transformer->resourceKey)){
            $itemResource = new Item($item, $transformer, $transformer->resourceKey);
        }
        else {
            $itemResource = new Item($item, $transformer);
        }
        return $this->fractalManager->createData($itemResource);
    }
    ...
    public function processItem($item){
        ...
        return $this->processItemWithTransformer($item, $this->transformer);
    }
    /**
     * @param $items
     * @return array|null
